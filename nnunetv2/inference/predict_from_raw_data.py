    ...
    def predict_from_files_sequential(self,
        ...
        if output_folder is not None:
            ...
            my_init_kwargs = deepcopy(
                my_init_kwargs)  # let's not unintentionally change anything in-place. Take this as a
            recursive_fix_for_json_export(my_init_kwargs)
            output_file = join(output_folder, 'predict_from_raw_data_args.json')
            os.makedirs(os.path.dirname(output_file), exist_ok=True)
            save_json(my_init_kwargs, output_file)
            # we need these two if we want to do things with the predictions like for example apply postprocessing
            save_json(self.dataset_json, join(output_folder, 'dataset.json'), sort_keys=False)
            save_json(self.plans_manager.plans, join(output_folder, 'plans.json'), sort_keys=False)
        #######################
...
if __name__ == '__main__':
    ...
    plan=os.environ['NNUNET_PLAN_CONFIG']
    NAME=os.environ['NaME']
    location1='Dataset0'+datasetid+'_'+NAME
    fOld = os.environ['fOld']
    example_raw = os.environ['example_raw']
    input_path = join(nnUNet_raw, location1, 'imagesTr', example_raw)
    file_lists = [[input_path], [input_path]]
    output_folder = join(nnUNet_raw, location1, 'imagesTs_predlowres')
    ...
    predictor.initialize_from_trained_model_folder(
       join(nnUNet_results, location1+'/nnUNetTrainer__nnUNetPlans__' + plan),
       use_folds=(fOld,),
       checkpoint_name='checkpoint_best.pth')


    # 确保输出目录存在
    os.makedirs(join(nnUNet_raw, location1 + '/imagesTs_predlowres'), exist_ok=True)
    predictor.predict_from_files(
        join(nnUNet_raw, location1 + '/imagesTs'),
        join(nnUNet_raw, location1 + '/imagesTs_predlowres'),
    ...
    img, props = SimpleITKIO().read_images([join(nnUNet_raw, location1 + '/imagesTr/'+example_raw)])
